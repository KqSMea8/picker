...
do
  ...
  equity=$(grep 'Total Equity:' $detl |cut -f2 -d':' |awk '{ print $1 }' |sed 's|n/a||g')
  net_income1=$(grep 'Profit after tax from continuing operations:' $detl |cut -f2 -d':' |awk '{ print $1 }' |sed 's|n/a||g')
  total_assets1=$(grep 'Total Assets:' $detl |cut -f2 -d':' |awk '{ print $1 }' |sed 's|n/a||g')
  total_liabilities1=$(grep 'Total Liabilities:' $detl |cut -f2 -d':' |awk '{ print $1 }'|sed 's|n/a||g')
  net_income2=$(grep 'Profit after tax from continuing operations:' $detl |cut -f2 -d':' |awk '{ print $2 }' |sed 's|n/a||g')
  total_assets2=$(grep 'Total Assets:' $detl |cut -f2 -d':' |awk '{ print $2 }' |sed 's|n/a||g')
  total_liabilities2=$(grep 'Total Liabilities:' $detl |cut -f2 -d':' |awk '{ print $2 }'|sed 's|n/a||g')
  net_income3=$(grep 'Profit after tax from continuing operations:' $detl |cut -f2 -d':' |awk '{ print $3 }' |sed 's|n/a||g')
  total_assets3=$(grep 'Total Assets:' $detl |cut -f2 -d':' |awk '{ print $3 }' |sed 's|n/a||g')
  total_liabilities3=$(grep 'Total Liabilities:' $detl |cut -f2 -d':' |awk '{ print $3 }'|sed 's|n/a||g')
  net_income4=$(grep 'Profit after tax from continuing operations:' $detl |cut -f2 -d':' |awk '{ print $4 }' |sed 's|n/a||g')
  total_assets4=$(grep 'Total Assets:' $detl |cut -f2 -d':' |awk '{ print $4 }' |sed 's|n/a||g')
  total_liabilities4=$(grep 'Total Liabilities:' $detl |cut -f2 -d':' |awk '{ print $4 }'|sed 's|n/a||g')
  net_income5=$(grep 'Profit after tax from continuing operations:' $detl |cut -f2 -d':' |awk '{ print $5 }' |sed 's|n/a||g')
  total_assets5=$(grep 'Total Assets:' $detl |cut -f2 -d':' |awk '{ print $5 }' |sed 's|n/a||g')
  total_liabilities5=$(grep 'Total Liabilities:' $detl |cut -f2 -d':' |awk '{ print $5 }'|sed 's|n/a||g')
  current_assets=$(grep -A 1 'Other Current Assets:' $detl |cut -f2 -d':' |awk '{ print $1 }' |tail -1|sed 's|n/a||g')
  current_liabilities=$(grep -A 1 'Other Current Assets:' $detl |cut -f2 -d':' |awk '{ print $1 }' |tail -1|sed 's|n/a||g')
  if [ -n "${total_assets1}" ]
  then
    echo "${name},${market_cap},${price},${pe_ratio},${volume},${equity},${net_income1},${total_assets1},${total_liabilities1},${net_income2},${total_assets2},${total_liabilities2},${net_income3},${total_assets3},${total_liabilities3},${net_income4},${total_assets4},${total_liabilities4},${net_income5},${total_assets5},${total_liabilities5},${current_assets},${current_liabilities}" >>./stock_details.csv
  fi
